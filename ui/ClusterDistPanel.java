...
public class ClusterDistPanel extends JPanel {
	...
	public ClusterDistPanel(networks.Graph graph, int displaySize) {
	...
	public void paintComponent(Graphics g) {
		...
		drawCenteredString(g, "Cluster Size", (x0 + length) / 2, y0 + labelWidth/2, bold);
		...
		List<Integer> clusterDist = new ArrayList<>();
		for (int size : clusterSizes) {
			while (clusterDist.size() < size) {
				clusterDist.add(0);
			}
			clusterDist.set(size - 1, clusterDist.get(size - 1) + 1);
		}
		double[] g0ps = clusterDist.stream().mapToDouble(
				i -> (double) i / graph.getClusters().size()).toArray();
